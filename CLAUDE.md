...
- 開発ドキュメント（`docs` ディレクトリ内のmdファイル）に実装要件が記載されているので実装前に内容を確認してください．
- ESLintルールに従ってください．
- any型は使用しないでください．
...
- 以下の操作は作業開始時に必ず行ってください．
  ...
  - 以下を必ず作業終了時に実行してください．
    ...
    2. `npm run lint`, `npm run format`, `npm run test:once` , `npm run build` を実行し，エラーのない状態にする．`npm run dev` は手動で実行するため不要．
    3. バックグラウンドで実行中のプロセスを終了させる．終了させないと多重起動状態になってPCの動作に影響する．
    4. 修正を行った場合は，修正後に必ず1から3の手順を再度実行する．
    5. 作業内容をコミットする．変更が大きい場合には一つの論理的な単位に分割してコミットする．
    6. ここで一度作業を停止してユーザのレビューを待つ．
    7. レビュー後，ドキュメントの追加・更新が必要な箇所について対応する．その後必ずフォーマットする．
    8. リモートブランチにpushする．
    9. プルリクエスト作成時には作業ブランチからdevelopブランチに対して作成する．
- 実装及び品質確認が完了したらレビューしますのでコミット前に作業を停止してください．
- 実装の際はコードが複雑にならないよう独断や推定で進めることは謹んでください．
- 実装中に想定と異なる状況に遭遇した場合はユーザの指示を仰いで下さい．
- 判断が必要な場合や仕様の確認が必要な場合は必ずユーザの指示を仰いで下さい．
- 統合テストなどは状況により要不要を判断する必要があるため迷う場合はユーザの指示を仰いで下さい．
- 実装のレビュー通過後に必ずドキュメントを更新します．docsディレクトリのREADME.md内「ドキュメント運用ルール」に運用ルールが記載されているため必ず参照して作業を行ってください．
